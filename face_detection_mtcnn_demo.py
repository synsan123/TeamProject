...
sys.path.append(
    '/home/kyj/openvino_notebooks/open_model_zoo/demos/common/python')
sys.path.append(
    '/home/kyj/openvino_notebooks/open_model_zoo/demos/common/python/model_zoo')
...
def build_argparser():
    ...
    args.add_argument("-ardu", "--device_ardu", default=None,
                      type=str, help="Arduino port")
...
def main():
    ...
    # ardu - face
    with FactoryController(args.device_ardu) as ctrl:
        FORCE_STOP = False
        ctrl.green = True
        while not FORCE_STOP:
            ...
            for i, scale in enumerate(scales):
                ...
                p_net.reshape(PartialShape([1, 3, ws, hs]))
                ...
                p_res = {name: infer_request_pnet.get_tensor(name).data[:] for name in {
                    pnet_roi_name, pnet_cls_name}}
            ...
            if len(rectangles) > 0:
                ...
                r_net.reshape(PartialShape([len(rectangles), 3, 24, 24]))
                ...
                for rectangle in rectangles:
                    crop_img = rgb_image[int(rectangle[1]):int(
                ...
                rnet_res = {name: infer_request_rnet.get_tensor(name).data[:] for name in {
                    rnet_roi_name, rnet_cls_name}}

                roi = rnet_res[rnet_roi_name]
                cls = rnet_res[rnet_cls_name]
                rectangles = utils.filter_face_24net(
                    cls, roi, rectangles, ow, oh, score_threshold[1], iou_threshold[2])

            # Onet stage
            if len(rectangles) > 0:
                # Change batch size of input blob
                o_net.reshape(PartialShape([len(rectangles), 3, 48, 48]))
                ...
                onet_res = {name: infer_request_onet.get_tensor(name).data[:] for name in {
                    onet_roi_name, onet_cls_name, onet_pts_name}}
            ...
            for rectangle in rectangles:
                ...
                for i in range(5, 15, 2):
                    cv2.circle(
                        origin_image, (int(rectangle[i+0]), int(rectangle[i+1])), 2, (0, 255, 0))
